...
public class AIPlayer extends Player{
	...
	/**
	 * This method return true if there is a valid place to moove
	 * @param w Width's Piece
	 * @param h Height's Piece
	 * @return false if Piece is stuck
	 */
	...
	/**
	 * This method return true if the Piece is on the final line
	 * @param h Height's Piece
	 * @return true if the Piece is on the arrival line
	 */
	private boolean isMarked(int h){
		boolean ret = false;
		if(((this.color == true) && h == this.board.getHeight()-1) || ((this.color == false) && h == 0)){
			ret = true;
		}
		return ret;
	}
	...
	public void playTurn(int turn){
		...
		while(!played){
			...
			if((this.board.getPiece(width, height) != null) && (this.board.getPiece(width, height).getColor() == this.color) && (this.isFree(width, height)) && !(isMarked(height))){
